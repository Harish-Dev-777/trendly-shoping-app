...
export default function StoreOrders() {
    ...
    const fetchOrders = async () => {
        if (!storeId) return
        try {
            const data = await fetchAPI(`/seller/${storeId}/orders`, { getToken })
            setOrders(data)
        } catch (error) {
    ...
    const updateOrderStatus = async (orderId, status) => {
        try {
            const promise = fetchAPI(`/orders/${orderId}/status`, {
                method: 'PUT',
    ...
    if (!storeId) return <div className="mt-10 text-center text-slate-500">You don't have a store yet. Please create a store to view your orders.</div>
