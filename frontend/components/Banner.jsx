...
export default function Banner() {
    ...
    useEffect(() => {
        const fetchCoupons = async () => {
            ...
            try {
                ...
                const newUserCoupon = coupons.find(c => c.forNewUser);
                if (newUserCoupon) {
                    setNewCoupon(newUserCoupon);
                }
            } catch (error) {
    ...
    const handleClaim = () => {
        setIsOpen(false);
        toast.success(`Coupon ${newCoupon.code} copied to clipboard!`);
        navigator.clipboard.writeText(newCoupon.code);
    };

    if (!newCoupon || !isOpen) return null;
    ...
    return (
        <div className="w-full px-6 py-1 font-medium text-sm text-white text-center bg-gradient-to-r from-violet-500 via-[#9938CA] to-[#E0724A]">
            <div className='flex items-center justify-between max-w-7xl mx-auto'>
                <p>Get {newCoupon.discount}% OFF on Your First Order! Use code: {newCoupon.code}</p>
