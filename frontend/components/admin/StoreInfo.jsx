...
const StoreInfo = ({store}) => {
    return (
        <div className="flex-1 space-y-2 text-sm">
            <Image width={100} height={100} src={store.logo || assets.upload_area} alt={store.name || "Store"} className="max-w-20 max-h-20 object-contain shadow rounded-full max-sm:mx-auto" />
            ...
            <div className="flex items-center gap-2 text-sm ">
                <Image width={36} height={36} src={store.user?.image || assets.upload_area} alt={store.user?.name || "User"} className="w-9 h-9 rounded-full" />
                <div>
